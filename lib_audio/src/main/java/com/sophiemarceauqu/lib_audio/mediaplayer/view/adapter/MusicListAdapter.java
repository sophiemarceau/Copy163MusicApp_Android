...
public class MusicListAdapter extends RecyclerView.Adapter {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new MyViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.dialog_bottom_sheet_item, null));
    }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        myViewHolder.author.setText(new StringBuilder().append(" ").append(bean.author));
        if (bean.id.equals(mCurrentBean.id)) {
            //为播放中歌曲，红色提醒
            myViewHolder.tip.setVisibility(View.VISIBLE);
            myViewHolder.name.setTextColor(mContext.getResources().getColor(android.R.color.holo_red_light));
            myViewHolder.author.setTextColor(mContext.getResources().getColor(android.R.color.holo_red_light));
        } else {
